...
The `data` property contains actual data server sends in the response to a request. You must use `setData` method to set
...
Also, data is considered empty when there is one item in the array and it is either `null` or `undefined`.

#### errors

The `errors` property is an array of objects with information about errors occured when processing a request.
The structure of objects in the `errors` array looks like this:
...
it is enough to call `result.didFail()` and then get error description for example like `result.getErrorSummary()`.

Alternatively, if you prefer work with exceptions you could do like this so works with errors is the same no matter
how an error occured.
